...
1. client_ip() took the left-most X-Forwarded-For value unconditionally,
   ...
   that header. It now reads REST_FRAMEWORK["NUM_PROXIES"] (1 in every
   service's settings.py, since every deployed environment sits behind
   exactly one reverse proxy), matching the same trusted-proxy semantics
   DRF's own throttling already uses.
...
class TestClientIp:
    ...
    def test_num_proxies_one_is_the_actual_configured_default(self):
        """Regression guard: the real settings.py value, not a value this
        test invents, must already trust exactly one hop -- Render's load
        balancer in every deployed environment -- and no more."""
        request = factory.get("/", REMOTE_ADDR="10.0.0.1", HTTP_X_FORWARDED_FOR="1.2.3.4")
