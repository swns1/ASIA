...
-- Demo data only. The lookup rows a real install also needs are in
-- scripts/reference_data.sql, which this file includes; for a real install
-- load that file on its own (scripts/setup-db.ps1 does, unless -Demo).
...
-- =====================================================
-- REFERENCE DATA
-- The requirement catalogue, grading templates and the DepEd observed-value
-- categories live in scripts/reference_data.sql, because a real install needs
-- them without any of the demo learners below. Included here (inside this
-- file's transaction) so loading the demo stays one command. It must come
-- before SUBJECTS, SCORE ENTRIES and NARRATIVE REPORTS, which reference it.
-- =====================================================
\ir scripts/reference_data.sql
...
-- =====================================================
-- NARRATIVE REPORTS
-- The categories themselves (IDs 900-903) come from
-- scripts/reference_data.sql; these narrative_reports (IDs 900+) cover the same
...
-- DO UPDATE: scripts/reference_data.sql (included above) has already created
-- a neutral singleton row, and the demo needs its own school year and
-- letterhead in place of it. Re-running this file therefore resets settings
-- edited in the portal, like the demo accounts above.
ON CONFLICT (setting_id) DO UPDATE SET
  current_school_year = EXCLUDED.current_school_year,
  sy_start_date       = EXCLUDED.sy_start_date,
  sy_end_date         = EXCLUDED.sy_end_date,
  early_bird_days     = EXCLUDED.early_bird_days,
