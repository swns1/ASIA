...
import { getStudents, getStudent } from "../api/studentApi";
import {
  createEnrollment,
  getEnrollment,
  updateEnrollment,
} from "../api/enrollmentApi";
import { getScholarshipTypes } from "../api/scholarshipApi";
import { createEnrollmentScholarship } from "../api/enrollmentScholarshipApi";

// ─── style tokens (mirrors StudentFormPage.jsx) ──────────────────────────────
const C = {
  red: "#e03131",
  redLight: "#fff0f0",
  redBorder: "#fca5a5",
  ...
  bg: "#fff8f6",
  white: "#ffffff",
  shadow: "0 4px 24px rgba(224,49,49,0.10)",
};

// ─── enum options (mirrors schema CHECK constraints) ─────────────────────────
const SCHOOL_LEVELS = [
...
// Grade level options derived from school_level
...
// Default school year (e.g. "2026-2027" if July+, otherwise "2025-2026")
...
function buildSchoolYearOptions() {
  const d = new Date();
  const baseYear = d.getMonth() >= 5 ? d.getFullYear() : d.getFullYear() - 1;
  const opts = [];
  for (let i = 1; i >= -2; i--) {
    const y = baseYear + i;
    opts.push(`${y}-${y + 1}`);
...
const getPalette = (name = "X") => AVATAR_PALETTES[name.charCodeAt(0) % AVATAR_PALETTES.length];

// ─── reusable form primitives (mirrors StudentFormPage) ──────────────────────
const inputStyle = {
  width: "100%",
...
function Field({ label, hint, children, required }) {
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {children}
      {hint && (
        <div style={{ fontSize: 11, color: C.muted, marginTop: 5, fontStyle: "italic" }}>
          {hint}
        </div>
      )}
    </div>
  ...
  return (
    <input
      ...
      style={{
        ...inputStyle,
        ...(focused
          ? { borderColor: C.red, boxShadow: `0 0 0 3px rgba(224,49,49,.10)`, background: C.white }
          : {}),
        ...style,
      }}
...
// ─── SectionCard (mirrors StudentDetailPage) ─────────────────────────────────
  ...
  return (
    <div
      style={{
        ...
        boxShadow: "0 2px 16px rgba(224,49,49,0.05)",
        overflow: "hidden",
...
// ─── Student picker (search + select existing student) ──────────────────────
function StudentPicker({ value, onChange, disabled }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const data = await getStudents({ page: 1, search: query, status: "active" });
  ...
  if (value) {
    const palette = getPalette(value.last_name ?? "X");
    const initials = `${value.first_name?.[0] ?? ""}${value.last_name?.[0] ?? ""}`.toUpperCase();
    const fullName = [value.first_name, value.middle_name, value.last_name, value.suffix]
      .filter(Boolean)
      .join(" ");
    ...
    return (
      <div
        style={{
          ...
          border: `1.5px solid ${C.redMid}`,
          borderRadius: 12,
          background: "linear-gradient(to right, #fff8f6, white)",
        }}
      >
        <div
          style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginTop: 2, display: "flex", gap: 12, flexWrap: "wrap" }}>
            {value.student_number && (
              <span><i className="ti ti-id-badge" style={{ fontSize: 11, marginRight: 3 }} />{value.student_number}</span>
            )}
            {value.lrn && (
              <span><i className="ti ti-fingerprint" style={{ fontSize: 11, marginRight: 3 }} />LRN {value.lrn}</span>
            )}
          </div>
        </div>
        {!disabled && (
      ...
      {open && query && (
        <div
          style={{
            ...
            borderRadius: 12,
            border: `1px solid ${C.redMid}`,
            boxShadow: "0 12px 40px rgba(224,49,49,0.14)",
            maxHeight: 320,
            overflowY: "auto",
            zIndex: 20,
          }}
        >
          ...
          {results.map((st) => {
            const palette = getPalette(st.last_name ?? "X");
...
export default function EnrollmentFormPage() {
  const { id }      = useParams();           // enrollment_id (edit mode)
  ...
  // Scholarship attachments (optional)
  const [scholarshipTypes, setScholarshipTypes]   = useState([]);
  const [selectedScholarships, setSelectedSchols] = useState([]); // array of scholarship_type_id
  const [scholarshipNotes, setScholarshipNotes]   = useState("");

  // ─── Load scholarship types once ───────────────────────────────────────────
  useEffect(() => {
    getScholarshipTypes()
      .then((data) => {
        const list = Array.isArray(data) ? data : data?.results ?? [];
        setScholarshipTypes(list.filter((s) => s.is_active));
      })
      .catch(() => setScholarshipTypes([]));
  }, []);

  // ─── Load enrollment for edit mode ─────────────────────────────────────────
  useEffect(() => {
    if (!isEdit) return;
    setLoading(true);
    getEnrollment(id)
  ...
  // ─── Cascading defaults on level change ────────────────────────────────────
  const gradeOptions = useMemo(
    () => GRADE_LEVELS_BY_LEVEL[form.school_level] ?? [],
    [form.school_level]
  );
  ...
  useEffect(() => {
    setForm((f) => {
      const next = { ...f };
      // Reset grade when level changes (if not in new options)
      if (!gradeOptions.includes(f.grade_level)) {
        next.grade_level = gradeOptions[0] ?? "";
      }
      // SHS-only fields
      if (isSHS) {
        if (!next.semester) next.semester = "1st";
      } else {
        next.strand = "";
        next.semester = "";
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        // Attach selected scholarships (only on create)
          ...
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 13, color: C.muted, marginTop: 4 }}>
                {isEdit
                  ? "Update class assignment, status, or term details."
                  : "Enroll an existing student into a school year and section."}
              </div>
            </div>
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <SectionCard title="Student" icon="ti-user-search">
              {isEdit ? (
                <>
                  <Field label="Enrolling Student">
                    <StudentPicker value={student} onChange={() => {}} disabled />
                  </Field>
                  <div style={{ fontSize: 12, color: C.muted, fontStyle: "italic" }}>
                    The student cannot be changed on an existing enrollment. Cancel this enrollment and create a new one if needed.
                  </div>
                </>
              ) : (
                <Field
                  ...
                  required
                  hint="Find an existing student record. Need to register a new one first? Use the Students page."
                >
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
                ...
                <Field label="Enrollment Status" required>
                  <Select
                    ...
                    onChange={(e) => setField("enrollment_status", e.target.value)}
                  >
            ...
            <SectionCard title="Class Assignment" icon="ti-school">
              {/* School level — visual chip selector */}
                  ...
                  {SCHOOL_LEVELS.map((lvl) => {
                    ...
                    return (
                      <button
                        key={lvl.value}
                        type="button"
                        onClick={() => setField("school_level", lvl.value)}
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
                          gap: 8,
                          padding: "9px 14px",
                          borderRadius: 99,
                          border: `1.5px solid ${active ? C.red : "#f0e4e4"}`,
                          background: active ? C.redLight : "white",
                      ...
                      >
                        <i className={`ti ${lvl.icon}`} style={{ fontSize: 14 }} />
                        {lvl.label}
                      </button>
                    );
                  })}
              ...
              {isSHS && (
                <div
                  style={{
                    marginTop: 6,
                    padding: "16px 18px",
                    background: "linear-gradient(to right, #fff8f6, #fff)",
                    border: `1px dashed ${C.redBorder}`,
                    borderRadius: 12,
                  }}
                >
                  <div style={{ fontSize: 11, color: C.red, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 12 }}>
                    Senior HS specifics
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
            ...
            {/* ── 4. Scholarships (create mode only) ─────────────────────── */}
            {!isEdit && (
              <SectionCard
                title="Scholarships"
                icon="ti-discount"
                badge={selectedScholarships.length > 0 ? selectedScholarships.length : null}
              >
                <div style={{ fontSize: 12, color: C.muted, marginBottom: 14, fontStyle: "italic" }}>
                  Optional. Attach any scholarships this enrollment qualifies for. You can manage these later from the Scholarships page.
                </div>

                {scholarshipTypes.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "20px 0", color: C.muted, fontSize: 13 }}>
                    No active scholarship types available.
                  </div>
                ) : (
                  <>
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              borderRadius: 12,
                              border: `1.5px solid ${active ? C.red : "#f0e4e4"}`,
                              background: active ? C.redLight : "#fffbfb",
                              cursor: "pointer",
                              transition: "all .14s",
                            }}
                          >
                            <div
                              style={{
                                ...
                                height: 20,
                                borderRadius: 6,
                                border: `2px solid ${active ? C.red : "#d0b8b8"}`,
                            ...
                            <span
                              style={{
                                ...
                                fontWeight: 700,
                                color: active ? C.red : C.muted,
                                background: active ? "white" : "#f9f4f4",
                                padding: "4px 10px",
                                borderRadius: 99,
                                whiteSpace: "nowrap",
                              }}
                      ...
                      <div style={{ marginTop: 14 }}>
                        <Field label="Notes" hint="Optional remarks applied to all selected scholarships.">
                          <Textarea
                            ...
                            placeholder="e.g. Approved by registrar on 2026-06-10"
                          />
                        </Field>
                      </div>
                    )}
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  letterSpacing: ".02em",
                  boxShadow: validationError ? "none" : "0 6px 20px rgba(224,49,49,0.28)",
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 8,
